...
of type `sink`. The technical differences between the two scenarios are highlighted in the xref:creating-sink[Creating a sink Kamelet] section.
...
As before, this integration can be tested with `camel run Earthquake.java` (the modeline parameters are automatically added by the kamel CLI).
...
a placeholder that is replaced with something else when the Kamelet is actually used (the user decides what is the destination of the earthquake events).
...
- from: # <4>
    ...
    - to: direct:endpoint
----
<1> Setting the `chatId` property directly in modeline, the `authorizationToken` is passed from command line
